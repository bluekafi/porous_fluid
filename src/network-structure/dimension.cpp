...
bool dst::Diamension::is_this_any_open_node(const int row, const int col) const
{
	return is_this_an_open_node(row, col) || is_this_an_injector_plate_node(row, col);
}


bool dst::Diamension::is_this_an_injector_plate_node(const int row, const int col) const
{
	if(row % 2)
	{
		return false;
