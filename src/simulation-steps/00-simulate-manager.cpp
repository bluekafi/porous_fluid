...
double CalculateBlue(const Tdouble& radius, const TMns& mnsc, const dst::Diamension& diamension)
{
	...
	while(total_volume_injected <=  proportion_of_injection * total_volume_of_system)
	{
		...
		const std::vector<double> pressure
			= func::Pressure::calculate_pressure(
				radius, mnsc, add_msn, diamension, total_flow_rate);
		...
		*/
		...
		config_at_moment_time.volume_blue_in_system = CalculateBlue(radius, mnsc, diamension);
