...
)
{
	...
	const int linear_node_input_pressure = diamension.total_nodes() - 1;
	for(int row = 0; row < diamension.node_rows(); ++ row)
	{
		for(int col = 0; col < diamension.node_cols(row); ++ col)
		{
			...
			if(diamension.is_this_an_injector_plate_node(row, col))
			{
				equation[linear_node] = 1;
	...
	//std::cout << "node_rows=" << diamension.node_rows() << std::endl;
	for(int row = 0; row < diamension.node_rows(); row += 2)
	{
		const int col = 0;
