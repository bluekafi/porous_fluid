...
{
	TFluid fluid_addition_table = dimension.empty_table_templated<Fluid>();
	std::vector<double> injection_vec;
	std::vector<double> expulsion_vec;

	for(int row = 0; row < dimension.node_rows(); ++ row)
	{
		const int max_cols_in_this_row = dimension.node_cols(row);
		for(int col = 0; col < max_cols_in_this_row; ++ col)
		{
			...
			*/
			...
			if(dimension.is_this_any_open_node(row, col))
			{
				if(vol_fluid_outgoing > vol_fluid_incoming)
				{
					...
					{
						const std::vector<double> add_fluid_vec
						{0, 2 * vol_fluid_to_be_injected_into_node};
						tank.add_fluid(add_fluid_vec);
					}
