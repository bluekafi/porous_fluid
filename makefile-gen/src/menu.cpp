...
{
	Print::makefile_command_title(fout, "all", {"necessary_compile", "run"});
	Print::echo(fout, "Command executed = all");
}

void Menu::run(std::ofstream& fout)
{
	Print::makefile_command_title(fout, "run");
	Print::echo(fout, "./" + decl::path_run_folder + "simulate");
}
...
{
	Print::makefile_command_title(fout, "force", {"clean", "necessary_compile"});
...
void Menu::clean(std::ofstream& fout)
{
	...
	Print::script(fout, decl::terminal_mkdir +  decl::path_build_folder);
}
...
{
	return decl::path_run_folder + split(file).second;
}
...
void Menu::edit(std::ofstream& fout, const std::vector<std::string>& file_vec)
{
	...
	Print::makefile_command_title(fout, "edit");
...
void Menu::exe(std::ofstream& fout, const std::vector<std::string>& file_vec)
{
	...
	for(std::string file_exe: exe_vec)
	{
		...
		Print::makefile_command_title(fout, Menu::convert_to_exe(file_exe), deps);
		Print::script(fout, decl::compile_command_exe + Menu::convert_to_str(deps) + " -o " + Menu::convert_to_exe(file_exe));
		Print::echo(fout, Menu::convert_to_object(file_exe) + " created.");
	}
}
...
void Menu::object(std::ofstream& fout, const std::vector<std::string>& file_vec)
{
	...
	for(const std::string& file: object_vec)
	{
		...
		Print::makefile_command_title(fout, Menu::convert_to_object(file), deps);
